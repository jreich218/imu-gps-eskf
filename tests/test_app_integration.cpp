...
void CopyBundledSceneFiles(const fs::path& working_dir) {
    ...
    const fs::path source_wheel_speed_path =
        source_scenarios_dir / "scene_zoe_veh_info.json";
    ...
    if (!fs::is_regular_file(source_wheel_speed_path)) {
        throw std::runtime_error("Missing bundled wheel-speed file: " +
                                 source_wheel_speed_path.string());
    }
    ...
    fs::copy_file(source_imu_path,
                  ...
                  fs::copy_options::overwrite_existing);
    fs::copy_file(source_wheel_speed_path,
                  scenarios_dir / "scene_zoe_veh_info.json",
                  fs::copy_options::overwrite_existing);
}
