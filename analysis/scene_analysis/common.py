...
LOCAL_PATHS_PATH = ANALYSIS_ROOT / "paths.json"


def load_local_paths() -> dict[str, object]:
    if not LOCAL_PATHS_PATH.is_file():
        raise RuntimeError(
            "Missing analysis/paths.json. Copy analysis/paths.example.json to "
            "analysis/paths.json and set the local paths there."
        )

    paths = json.loads(LOCAL_PATHS_PATH.read_text())
    if not isinstance(paths, dict):
        raise RuntimeError(
            f"Unexpected JSON root in {LOCAL_PATHS_PATH}. Expected an object."
        )
    return paths


def local_can_bus_dir() -> Path:
    paths = load_local_paths()
    can_bus_dir = paths.get("can_bus_dir")
    if can_bus_dir is None:
        raise RuntimeError(
            "analysis/paths.json must define the string field "
            '"can_bus_dir".'
        )
    if not isinstance(can_bus_dir, str):
        raise RuntimeError(
            "analysis/paths.json field "
            '"can_bus_dir" must be a string.'
        )

    path = Path(can_bus_dir).expanduser()
    if not path.is_dir():
        raise RuntimeError(f'Configured "can_bus_dir" does not exist: {path}')
    return path
...
def list_scene_ids() -> list[str]:
    scene_ids = [
        line.strip()
        for line in SUPPORTED_SCENES_PATH.read_text().splitlines()
        if line.strip()
    ]
    if not scene_ids:
        raise RuntimeError(f"No scene IDs found in {SUPPORTED_SCENES_PATH}.")
    return scene_ids


def make_scene_paths(scene_id: str) -> tuple[Path, Path, Path]:
    can_bus_dir = local_can_bus_dir()
    return (
...
def current_run_label() -> str:
    branch = current_branch_name().replace("/", "_")
