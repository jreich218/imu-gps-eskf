...
struct StartupTraceFrame {
    ...
    Eigen::Vector2d fitted_endpoint_xy{0.0, 0.0};  ///< Fitted path endpoint position.
    Eigen::Vector2d fitted_velocity_xy{0.0, 0.0};  ///< Fitted path endpoint velocity.
...
struct StartupTraceResult {
    ...
    std::vector<StartupTraceFrame> frames;  ///< One frame per tested startup prefix.
 ...
 *
 * The returned trace records the quantities the startup code computes for
