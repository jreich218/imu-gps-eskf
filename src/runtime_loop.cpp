...
EskfRunResult RunEskfLoop(const LoadedScene& loaded_scene,
    ...
    for (std::size_t imu_index =
        ...
        while (gps_index < gps_samples.size() &&
               gps_samples[gps_index].utime <= current_imu_sample.utime) {
            const GpsSample& gps_sample = gps_samples[gps_index];
            const PoseSample& pose_sample = PoseSampleAtTimestamp(
                loaded_scene.pose_samples, pose_index, gps_sample.utime);
